  ...
  module V1
   class CourseBattlesController < ApplicationController
    ...
    def create
      ...
      def get_messages
        begin
        messages = CourseBattleMessage.select("id, user_id as userId, course_battle_id as courseBattleId, message, created_at as createdAt").where({course_battle_id:params[:course_battle_id]})
        render json: {
          status: 200,
